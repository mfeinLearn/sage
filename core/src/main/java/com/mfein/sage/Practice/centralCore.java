 ...
 * This abstract class is the foundation of all practice modes.
 */
public abstract class centralCore extends DefaultScreen {
    ...
    /**
     * Constructor for centralCore.
     * @param gameInstance Main game instance.
     * @param c Level name, also the correct answer.
     * @param list List of all levels in the stage.
     * @param size Number of choices for the level.
     * @param lim Number of times the player needs to get the right answer.
     * @param start Starting stage position.
     * @param end Ending stage position.
     */
    public centralCore(Main gameInstance, String c, ArrayList<String> list, int size, int lim, int start, int end) {
        super();
        this.main = gameInstance;
        this.character = c;
        this.arr = new ArrayList<>(list); // Create a copy to avoid modifying the input list
        this.limit = lim;
        this.start = start;
        this.end = end;
        ...
        this.skin2 = new Skin(Gdx.files.internal("skin/uiskin.json"));
        this.font = new BitmapFont();
        this.font.getData().setScale(Constants.gameWidth(6f), Constants.gameHeight(6f));
        ...
        this.textButtons = new TextButton[size];
        this.imageButtons = new ImageButton[size];

        // Initialize textures
        this.correctTexture = AssetManager.getInstance().getTexture("happyFace.png");
        this.incorrectTexture = AssetManager.getInstance().getTexture("sadFace.png");
        this.gemsTexture = new Texture(Gdx.files.internal("Chest_Gem.png"));
        ...
        // Remove correct answer from arr to avoid duplicates in choices
        this.arr.remove(character);

        // Set positions and randomize choices
        setPositions();
        randomize();
    ...
    /**
     * Initializes the mainContainer and sets the background.
     */
    ...
    /**
     * Abstract method to set positions for UI elements.
     */
    protected abstract void setPositions();

    /**
     * Creates a new ImageButton style from a texture.
     */
    ...
    /**
     * Sets up an ImageButton for a choice.
     */
    private void setImageButton(final int i) {
        imageButtons[i] = new ImageButton(AssetManager.getInstance().convertTextureToDrawable(choices[i] + ".png"));
    ...
    /**
     * Sets up a TextButton for a choice.
     */
    ...
    private void create() {
        ...
        ImageButton exitButton = new ImageButton(AssetManager.getInstance().convertTextureToDrawable("Exit.png"));
    ...
    /**
     * Abstract method to initialize extra assets.
     */
    protected abstract void createExtraAssets();

    /**
     * Abstract method to set listeners for ImageButtons.
     */
     ...
     * Draws the score UI.
     */
    private void drawScore(SpriteBatch spriteBatch, Texture t, float x, float y, float w, float h) {
    ...
    /**
     * Abstract method to render extra textures or animations.
     */
    ...
    /**
     * Abstract method to dispose of extra assets.
     */
    protected abstract void disposeExtraAssets();

    /**
     * Abstract method to handle correct choice.
     */
    ...
    private void choiceMade(String choice) {
        ...
        if (character.equals(choice)) {
            ...
            if (timesCorrect <= limit) {
                viewDialog(AssetManager.getInstance().convertTextureToDrawable("happyFace.png"));
            }
            if (timesCorrect > limit) {
                numGems++;
                float delay = .75f;
                Timer.schedule(new Timer.Task() {
                    @Override
                    public void run() {
                        main.setScreen(new writingScreen(main, 10 - (0.1 * numWrong), skin, skin2, character, start, end));
                    }
                }, delay);
            } else {
                change();
    ...
    /**
     * Shows a dialog with the given drawable.
     */
...
// * */
//
//public class centralCore extends DefaultScreen {
//
//
//    protected SpriteBatch batch;
//    protected Stage stage, stage2;
